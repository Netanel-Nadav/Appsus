...
export const bookService = {
  ...
  removeBook,
  saveReview,
  saveGoogleBooksToStorage
}


const STORAGE_KEY = 'bookDB'
const STORAGE_GOODLE = 'googleBooksDB'
...
function query(filterBy = null, isGoogleBooks = false) {
  const books = isGoogleBooks ? _loadGoogleBooksFromStorage() : _loadBooksFromStorage();
  if (!filterBy) return Promise.resolve(books);
  const filteredBooks = isGoogleBooks ? _getFilterdGoogleBooks(books, filterBy) : _getFilterdBooks(books, filterBy);
...
function _getFilterdGoogleBooks(books, filterBy) {
...
function _getFilterdBooks(books, filterBy) {
...
function saveReview(bookId, review) {
  ...
  books[bookIdx].reviews = [review, ...books[bookIdx].reviews]
...
function _createBooks() {
  ...
  if (!books || books.lengthh) {
    books = gBooks;
    _saveBooksToStorage(STORAGE_KEY, books)
  }
}

function saveGoogleBooksToStorage(googleBooks) {
  _saveBooksToStorage(STORAGE_GOODLE, googleBooks);
}

function _saveBooksToStorage(key, books) {
...
function _loadGoogleBooksFromStorage() {
  return storageService.loadFromStorage(STORAGE_GOODLE);
}